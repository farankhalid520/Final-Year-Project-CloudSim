...
public abstract class PowerVmAllocationPolicyMigrationAbstract extends PowerVmAllocationPolicyAbstract {
	...
	/* VM Placement PEFFD
	 *  public PowerHost findHostForVm(Vm vm, Set<? extends Host> activeHostList) {
		double bestPowerEfficiency = Double.MIN_VALUE;
		PowerHost allocatedHost = null;

		for (PowerHost host : this.<PowerHost> getHostList()) {
			if (activeHostList.contains(host)) {
				continue;
			}
			if (host.isSuitableForVm(vm)) {
				double powerEfficiency = host.getMaxAvailableMips()/host.getMaxPower();
			...
			if (host.isSuitableForVm(vm)) 
			{
				...
				else
				{
					if(powerEfficiency == bestPowerEfficiency)
						...
						if(host.getAvailableMips() < allocatedHost.getAvailableMips())
						{
							allocatedHost = null;
						}
				}
	 ...
	 * public PowerHost findHostForVm(Vm vm, Set<? extends Host> excludedHosts) {
		...
		for (PowerHost host : this.<PowerHost> getHostList()) {
			...
			if (host.isSuitableForVm(vm)) {
				...
				diff = Math.abs(host.getMaxUtilization() - desiredLevel);
				if(diff < minPower)
				{
					allocatedHost = host;
					minPower = diff;
				...
				else
				{
					if(diff == minPower)
					{
						if(host.getMaxPower() > allocatedHost.getPower())
							allocatedHost = host;
					}
				}
	 ...
	 * public PowerHost findHostForVm(Vm vm, Set<? extends Host> excludedHosts) {
		...
		for (PowerHost host : this.<PowerHost> getHostList()) {
			...
			if (host.isSuitableForVm(vm)) {
				double cpu = host.getMaxAvailableMips();
				if(cpu < minCPU)
				{
					allocatedHost = host;
					minCPU = cpu;
				...
				{
					if(cpu == minCPU && host.getRam() < allocatedHost.getRam())
						allocatedHost = host;
				}
			}
		}
	...
	/*VM Placement Modified-MFPED
	 * public PowerHost findHostForVm(Vm vm, Set<? extends Host> excludedHosts) {
		double minPower = Double.MAX_VALUE;
		PowerHost allocatedHost = null;
		double desiredLevel = 0.8;
		double diff = 0;
		...
		for (PowerHost host : this.<PowerHost> getHostList()) {
			...
			if (host.isSuitableForVm(vm)) {
				...
				else
				{
					if(diff == minPower)
					{
						if(host.getMaxAvailableMips() > allocatedHost.getMaxAvailableMips && 
						host.getMaxPower() > allocatedHost.getPower())
							allocatedHost = host;
					}
				}
	 ...
	 */
	public PowerHost findHostForVm(Vm vm, Set<? extends Host> excludedHosts) { //default PABFD algo
		double minPower = Double.MAX_VALUE;
		PowerHost allocatedHost = null;
